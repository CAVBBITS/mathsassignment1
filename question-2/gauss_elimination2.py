...
def partial_pivot(a_matrix:np.array, head_row_index:int)->np.array:
    """
...
def elimination(a_matrix):
    ...
    for i in range(0, len(a_matrix) - 1, 1):
        cc = i
        a_matrix = partial_pivot(a_matrix,i)
        for nr in range (i+1, len(a_matrix), 1):
            coefficient = a_matrix[nr][cc] / a_matrix[i][cc]
            for cc1 in range(cc, len(a_matrix[0]), 1):
                a_matrix[nr, cc1] = a_matrix[nr, cc1] - (coefficient * a_matrix[i, cc1])
...
def guass_elimination(aug_matrix:np.array)->np.array:
    ...
    try:
        aug_matrix      = elimination(aug_matrix)
        output_x_vector = back_substituion(aug_matrix)
