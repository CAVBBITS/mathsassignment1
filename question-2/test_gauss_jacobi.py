...
class Test(TestCase):
    ...
    def test_is_nn_matrix_diagnolly_dominant(self):
        gaussJacobi = GaussJacobi (3,0,0.001)
        matrix      = np.array([86,20,33.0,3.0,91,96,53,97,19,88,80,95.0,83., 33.,  8.,  4.]).reshape(4,4)
        isDominent  = gaussJacobi.is_nn_matrix_diagnolly_dominant(matrix)
        print(f"isDominent={isDominent}")
        assert(isDominent==False)

    def test_is_nn_matrix_diagnolly_dominant_2(self):
        gaussJacobi = GaussJacobi(3, 0, 0.001)
